...
def test_users(db):  # db fixture ensures database access
    """Create test users."""
    return {
        'banned_user': User.objects.create(username='banned_user', email='banned@example.com'),
...
class TestDiscussionBanModel:
    ...
    def test_multiple_inactive_bans_allowed(self, test_users, test_course_keys):
        ...
        assert DiscussionBan.objects.filter(
            user=test_users['banned_user'],
            course_id=test_course_keys['harvard_cs50'],
            is_active=False
        ).count() == 2
    ...
    def test_clean_validation_org_scope_requires_org_key(self, test_users):
        ...
        with pytest.raises(ValidationError, match="Organization-level bans require organization"):
            ban.clean()

    def test_clean_validation_org_scope_cannot_have_course_id(self, test_users, test_course_keys):
        ...
        with pytest.raises(ValidationError, match="Organization-level bans should not have course_id set"):
            ban.clean()
    ...
    def test_is_user_banned_course_level(self, test_users, test_course_keys):
        ...
        assert DiscussionBan.is_user_banned(
            test_users['banned_user'],
            test_course_keys['harvard_cs50']
        ) is True

        # User should NOT be banned in Math101
        assert DiscussionBan.is_user_banned(
            test_users['banned_user'],
            test_course_keys['harvard_math']
        ) is False

        # Another user should NOT be banned
        assert DiscussionBan.is_user_banned(
            test_users['another_user'],
            test_course_keys['harvard_cs50']
        ) is False
    ...
    def test_is_user_banned_with_course_id_as_string(self, test_users, test_course_keys):
        ...
        assert DiscussionBan.is_user_banned(
            test_users['banned_user'],
            str(test_course_keys['harvard_cs50'])
        ) is True
...
class TestDiscussionBanExceptionModel:
    ...
    def test_exception_only_for_org_bans_validation(self, test_users, test_course_keys):
        ...
        with pytest.raises(ValidationError, match="Exceptions can only be created for organization-level bans"):
            exception.clean()
...
# ==================== DiscussionModerationLog Model Tests ====================


@pytest.mark.django_db
class TestDiscussionModerationLogModel:
    """Tests for DiscussionModerationLog model."""

    def test_create_ban_log_entry(self, test_users, test_course_keys):
        """Test creating a ban action log entry."""
        log = DiscussionModerationLog.objects.create(
            action_type=DiscussionModerationLog.ACTION_BAN,
            target_user=test_users['banned_user'],
            moderator=test_users['moderator'],
            course_id=test_course_keys['harvard_cs50'],
            scope='course',
            reason='Posting spam',
            metadata={'threads_deleted': 5, 'comments_deleted': 12},
        )

        assert log.action_type == 'ban_user'
        assert log.target_user == test_users['banned_user']
        assert log.moderator == test_users['moderator']
        assert log.course_id == test_course_keys['harvard_cs50']
        assert log.scope == 'course'
        assert log.reason == 'Posting spam'
        assert log.metadata == {'threads_deleted': 5, 'comments_deleted': 12}
        assert log.created is not None

    def test_create_unban_log_entry(self, test_users, test_course_keys):
        """Test creating an unban action log entry."""
        log = DiscussionModerationLog.objects.create(
        ...
        assert log.action_type == 'unban_user'

    def test_create_exception_log_entry(self, test_users, test_course_keys):
        """Test creating a ban exception log entry."""
        log = DiscussionModerationLog.objects.create(
            ...
            course_id=test_course_keys['harvard_cs50'],
            scope='organization',
            reason='Exception created for CS50',
            metadata={'ban_id': 123, 'organization': 'HarvardX'},
        )

        assert log.action_type == 'ban_exception'
        assert log.metadata['organization'] == 'HarvardX'

    def test_create_bulk_delete_log_entry(self, test_users, test_course_keys):
        """Test creating a bulk delete log entry."""
        log = DiscussionModerationLog.objects.create(
            action_type=DiscussionModerationLog.ACTION_BULK_DELETE,
            target_user=test_users['banned_user'],
            moderator=test_users['moderator'],
            course_id=test_course_keys['harvard_cs50'],
            reason='Spam cleanup',
        ...
        assert log.action_type == 'bulk_delete'
        assert log.metadata['task_id'] == 'abc-123'

    def test_log_str_representation(self, test_users, test_course_keys):
        ...
        expected = f"ban_user: {test_users['banned_user'].username} by {test_users['moderator'].username}"
        assert str(log) == expected

    def test_log_str_representation_no_moderator(self, test_users, test_course_keys):
        """Test string representation when moderator is None (system action)."""
        log = DiscussionModerationLog.objects.create(
            action_type=DiscussionModerationLog.ACTION_BULK_DELETE,
            target_user=test_users['banned_user'],
        ...
        expected = f"bulk_delete: {test_users['banned_user'].username} by System"
        assert str(log) == expected

    def test_log_ordering_by_created_desc(self, test_users, test_course_keys):
        """Test that logs can be ordered by created timestamp."""
        # Create multiple log entries
        log1 = DiscussionModerationLog.objects.create(
            action_type=DiscussionModerationLog.ACTION_BAN,
            target_user=test_users['banned_user'],
            moderator=test_users['moderator'],
            course_id=test_course_keys['harvard_cs50'],
        ...
        log2 = DiscussionModerationLog.objects.create(
            action_type=DiscussionModerationLog.ACTION_UNBAN,
            target_user=test_users['banned_user'],
            moderator=test_users['moderator'],
            course_id=test_course_keys['harvard_cs50'],
        ...
        # Query ordered by created descending
        logs = DiscussionModerationLog.objects.filter(
            target_user=test_users['banned_user']
        ).order_by('-created')

        assert list(logs) == [log2, log1]

    def test_moderator_set_null_on_delete(self, test_users, test_course_keys):
        """Test that moderator is set to NULL when user is deleted."""
        log = DiscussionModerationLog.objects.create(
            action_type=DiscussionModerationLog.ACTION_BAN,
            target_user=test_users['banned_user'],
            moderator=test_users['moderator'],
        ...
        # Delete moderator
        test_users['moderator'].delete()

        # Log should still exist with moderator=None
        log_reloaded = DiscussionModerationLog.objects.get(id=log_id)
        assert log_reloaded.moderator is None
        assert log_reloaded.target_user == test_users['banned_user']

    def test_log_cascade_delete_with_target_user(self, test_users, test_course_keys):
        """Test that logs are deleted when target user is deleted."""
        log = DiscussionModerationLog.objects.create(
            action_type=DiscussionModerationLog.ACTION_BAN,
            target_user=test_users['banned_user'],
            moderator=test_users['moderator'],
        ...
        # Delete target user
        test_users['banned_user'].delete()

        # Log should be deleted
        assert not DiscussionModerationLog.objects.filter(id=log_id).exists()
