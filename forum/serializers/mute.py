...
class MuteInputSerializer(serializers.Serializer):
    ...
    muter_id = serializers.CharField(
        required=True,
        help_text="ID of user performing the mute action"
    )
...
class UnmuteInputSerializer(serializers.Serializer):
    """Serializer for unmute input data."""
    
    unmuter_id = serializers.CharField(
        required=True,
        help_text="ID of user performing the unmute action"
    )
    scope = serializers.ChoiceField(
        choices=DiscussionMute.Scope.choices,
        default=DiscussionMute.Scope.PERSONAL,
...
class MuteAndReportInputSerializer(serializers.Serializer):
    """Serializer for mute and report input data."""
    
    muter_id = serializers.CharField(
        required=True,
        help_text="ID of user performing the mute and report action"
    )
    scope = serializers.ChoiceField(
        choices=DiscussionMute.Scope.choices,
        default=DiscussionMute.Scope.PERSONAL,
...
class UserMuteStatusSerializer(serializers.Serializer):
    ...
    muted_by_id = serializers.CharField(
        allow_null=True,
        help_text="ID of user who muted this user (for personal mutes)"
    )
    muted_by_username = serializers.CharField(
        allow_null=True,
        help_text="Username of user who muted this user"
    )
    muted_at = serializers.DateTimeField(
        allow_null=True,
        help_text="When the user was muted"
    )
    reason = serializers.CharField(
        allow_null=True,
    ...
    has_exception = serializers.BooleanField(
        default=False,
        help_text="Whether viewer has an exception for course-wide mutes"
    )


class MutedUserSerializer(serializers.Serializer):
    ...
    muted_by_username = serializers.CharField(help_text="Username of user who performed the mute")
    ...
    is_active = serializers.BooleanField(help_text="Whether the mute is currently active")


class CourseMutedUsersSerializer(serializers.Serializer):
    """Serializer for course-wide muted users list response."""
    
    course_id = serializers.CharField(help_text="Course ID")
    requester_id = serializers.CharField(
        allow_null=True,
        help_text="ID of user requesting the list"
    )
    ...
    muted_users = MutedUserSerializer(
        many=True,
        help_text="List of muted users"
    )


class DiscussionMuteSerializer(serializers.ModelSerializer):
    """Serializer for DiscussionMute model."""
    
    muted_user_id = serializers.CharField(source='muted_user.pk', read_only=True)
    muted_user_username = serializers.CharField(source='muted_user.username', read_only=True)
    muted_by_id = serializers.CharField(source='muted_by.pk', read_only=True)
    muted_by_username = serializers.CharField(source='muted_by.username', read_only=True)
    unmuted_by_id = serializers.CharField(source='unmuted_by.pk', read_only=True, allow_null=True)
    unmuted_by_username = serializers.CharField(source='unmuted_by.username', read_only=True, allow_null=True)
    
    class Meta:
        model = DiscussionMute
        fields = [
            'id', 'muted_user_id', 'muted_user_username',
            'muted_by_id', 'muted_by_username',
            'unmuted_by_id', 'unmuted_by_username',
            'course_id', 'scope', 'reason', 'is_active',
            'created', 'modified', 'muted_at', 'unmuted_at'
        ]
        read_only_fields = ['id', 'created', 'modified', 'muted_at', 'unmuted_at']


class DiscussionMuteExceptionSerializer(serializers.ModelSerializer):
    """Serializer for DiscussionMuteException model."""
    
    muted_user_id = serializers.CharField(source='muted_user.pk', read_only=True)
    muted_user_username = serializers.CharField(source='muted_user.username', read_only=True)
    exception_user_id = serializers.CharField(source='exception_user.pk', read_only=True)
    exception_user_username = serializers.CharField(source='exception_user.username', read_only=True)
    
    class Meta:
        model = DiscussionMuteException
        fields = [
            'id', 'muted_user_id', 'muted_user_username',
            'exception_user_id', 'exception_user_username',
            'course_id', 'created', 'modified'
        ]
...
class ModerationAuditLogSerializer(serializers.ModelSerializer):
    ...
    moderator_id = serializers.CharField(source='moderator.pk', read_only=True, allow_null=True)
    moderator_username = serializers.CharField(source='moderator.username', read_only=True, allow_null=True)
    
    class Meta:
        model = ModerationAuditLog
        fields = [
            'id', 'timestamp', 'body', 'classifier_output', 'reasoning',
            'classification', 'actions_taken', 'confidence_score',
            'moderator_override', 'override_reason',
