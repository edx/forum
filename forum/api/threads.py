...
def soft_delete_thread(
    ...
    # Update user stats: increment deleted counts (total threads count stays the same)
    author_id = thread.get("author_id")
    thread_course_id = thread.get("course_id")
    if author_id and thread_course_id:
        backend.update_stats_for_course(author_id, thread_course_id, deleted_threads=1, deleted_count=1)
...
def restore_thread(
    ...
    # Check if not soft deleted (treat NULL/missing as False for legacy data)
    ...
    # Update user stats: decrement deleted counts (total threads count stays the same)
    author_id = thread.get("author_id")
    thread_course_id = thread.get("course_id")
    if author_id and thread_course_id:
        backend.update_stats_for_course(author_id, thread_course_id, deleted_threads=-1, deleted_count=-1)
...
def soft_delete_thread(thread_id: str, user_id: str, course_id: Optional[str] = None) -> dict[str, Any]:
    ...
    # Perform soft delete (user_id is already a string)
    success = backend.soft_delete_thread(thread_id, user_id)
    if not success:
        raise ForumV2RequestError(f"Failed to soft delete thread {thread_id}")
...
def bulk_soft_delete_threads(thread_ids: list[str], user_id: str, course_id: Optional[str] = None) -> dict[str, Any]:
    ...
    # Perform bulk soft delete (user_id is already a string)
    try:
        count = backend.bulk_soft_delete_threads(thread_ids, user_id)
